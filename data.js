...
const fs = require('fs')
const PATH = '../server/api/group/comments/'
...
if (process.env.RESET) {
  ...
  fs.writeFileSync(PATH + 'slim.json', '{}')
}

// Have to do this because the module system is retarded.
let comments = JSON.parse(fs.readFileSync(PATH + 'comments.json'))
let slim = JSON.parse(fs.readFileSync(PATH + 'slim.json'))
